...
### Added
- Достигнут паритет с PortProton
- Добавлена `classic`-тема, максимально близкая по внешнему виду к PortProton, чтобы переход на PortProtonQt был привычным и не вызывал лишних вопросов у пользователей
- В настройках добавлена галочка экономного режима, которая полностью отключает все сетевые запросы

### Fixed
- Подсказки в настройках теперь показываются только когда текст обрезан, а не всегда
- Исправлено лишнее пустое пространство в выпадающем списке QComboBox за счёт установки минимальной высоты элементов списка
