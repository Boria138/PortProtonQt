...
- Эксперементальная поддержка ARM через запуск в muvm
- Аргумент командной строки --force-muvm (автоматический запуск в muvm происходит только при наличии Apple Silocon)
- Утилита vk_gpu_info для использовнии в логе
...
- Измена структура тем для поддержки вложенныхх папок
- После скачивания Wine список выбранных Wine теперь очищается
- Эмуляция мыши через геймпад теперь не зависит от окна в фокусее и работает всегда
