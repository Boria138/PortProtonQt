...
- Исправлено определение версии DXVK в логе если у игры нет ppdb
- Исправлено обновление библиотеки после создания ярлыков PortProton
- Исправлено обновление пустой библиотеки после запуска игры через `--silent`
- Исправлено использование обложек автоустановок для тем с режимом библиотеки `list`
...
- Исправлена установка Wine/LG-сборок в `data/dist` независимо от настройки загрузки Proton в Steam
- Исправлен запуск PortProtonQt в полноэкранном режиме, если геймпад подключён до старта приложения
- Исправлен сброс настроек PortProtonQt: теперь также сбрасывается `user.conf` PortProton
- Исправлено отображение префиксов `DEFAULT`, `DOTNET` и `PROGRAMS` при отсутствии соответствующих каталогов
