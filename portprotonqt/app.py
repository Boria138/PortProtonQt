...
import pygame  # Импорт для поддержки геймпада
...
def load_pixmap(cover, width, height):
    ...
    После масштабирования с KeepAspectRatioByExpanding происходит обрезка центральной части.
    """
...
class VirtualKeyboard(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        header_label.setStyleSheet("color: white; font-size: 18px; font-family: 'Poppins';")
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.game_processes = []
        # Для навигации с геймпада
        self.game_cards = []       # Список карточек (GameCard)
        self.focused_card_index = None
        self.grid_columns = 4      # Число колонок в сетке
        ...
        # Инициализация поддержки геймпада через pygame
        self.initGamepad()
    ...
    def pollGamepad(self):
        ...
        for event in pygame.event.get():
            # Обработка цифровой навигации через D-Pad
            if event.type == pygame.JOYHATMOTION:
                ...
                if hat_value[1] == 1:
                    ...
                    self.navigateDown()
            # Обработка аналоговых стиков (левый и правый)
            elif event.type == pygame.JOYAXISMOTION:
                # Горизонтальное перемещение: оси 0 (левый стик) и 2 (правый стик)
                if event.axis in (0, 2):
                    if event.value > 0.5 and current_time - self.last_axis_move_x > self.axis_move_delay:
                        ...
                        self.navigateLeft()
                        self.last_axis_move_x = current_time
                # Вертикальное перемещение: оси 1 (левый стик) и 3 (правый стик)
            ...
            # Обработка нажатия кнопок
                ...
                # Кнопка 0 ("A") для активации выбранной карточки
                if event.button == 0:
                    self.activateFocusedWidget()
                # Кнопка 1 ("B") для возврата из детального режима
                elif event.button == 1:
                    if self.stackedWidget.currentIndex() != 0 and hasattr(self, "currentDetailPage"):
                        self.goBackDetailPage(self.currentDetailPage)
                        del self.currentDetailPage

    def navigateRight(self):
        if self.stackedWidget.currentIndex() != 0 or not self.game_cards:
            return
        if self.focused_card_index is None:
            self.focused_card_index = 0
        else:
            new_index = self.focused_card_index + 1
            if new_index < len(self.game_cards):
                self.focused_card_index = new_index
        self.game_cards[self.focused_card_index].setFocus()
        if hasattr(self, 'gamesScrollArea'):
            self.gamesScrollArea.ensureWidgetVisible(self.game_cards[self.focused_card_index], 20, 20)
    ...
    def navigateDown(self):
        ...
        if self.focused_card_index is None:
            ...
            new_index = self.focused_card_index + self.grid_columns
            if new_index < len(self.game_cards):
                self.focused_card_index = new_index
        self.game_cards[self.focused_card_index].setFocus()
        if hasattr(self, 'gamesScrollArea'):
    ...
    def activateFocusedWidget(self):
        if self.stackedWidget.currentIndex() == 0 and self.game_cards and self.focused_card_index is not None:
            card = self.game_cards[self.focused_card_index]
            card.select_callback(card.name, card.description, card.cover_path, card.appid, card.exec_line)
    ...
    def createInstalledTab(self):
        ...
        self.gamesScrollArea = QtWidgets.QScrollArea()
        self.gamesScrollArea.setWidgetResizable(True)
        self.gamesScrollArea.setStyleSheet("border: none;")
        self.gamesScrollArea.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        ...
        self.gamesScrollArea.setWidget(listWidget)
        layout.addWidget(self.gamesScrollArea)
    ...
    def populateGamesGrid(self, games_list):
        ...
        self.game_cards = []  # Обновляем список карточек
        self.focused_card_index = None  # Сброс фокуса
    ...
    def openGameDetailPage(self, name, description, cover_path=None, appid="", exec_line=""):
        ...
        # Сохраняем текущую детальную страницу для обработки кнопки "B"
...
if __name__ == '__main__':
    ...
    load_custom_fonts()  # Загрузка кастомных шрифтов, включая RASKHAL
    window = MainWindow()
    window.show()
    sys.exit(app.exec())
