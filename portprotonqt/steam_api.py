    ...
    if " " in candidate_norm:
        for name_norm, app in steam_apps_index.items():
            if candidate_norm in name_norm:
                logger.debug("    Найдено частичное совпадение: кандидат '%s' в '%s'", candidate_norm, name_norm)
                return app
    else:
        logger.debug("    Кандидат '%s' состоит из одного слова, частичное совпадение не используется", candidate_norm)
