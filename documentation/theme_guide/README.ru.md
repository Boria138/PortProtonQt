...
| `click` | Нажатие кнопок и кликабельных меток |
| `confirm` | Подтверждение действия |
| `back` | Возврат или закрытие действием Back |
| `toggle` | Изменение флажка или переключателя |
| `open` | Открытие страниц, диалогов и меню |
| `close` | Закрытие элементов интерфейса |
| `error` | Сообщение об ошибке |
| `notification` | Уведомления |
| `keyboard_key` | Ввод с виртуальной клавиатуры |
| `scroll` | Обратная связь при прокрутке |
| `tab_switch` | Переключение вкладок приложения |
| `game_launch` | Успешный запуск процесса игры |
| `gamepad_connect` | Подключение геймпада |

Например, используйте `sounds/navigate.ogg` или `sounds/navigate.wav`. Поддерживаемые расширения в порядке поиска: `.ogg`, `.oga`, `.wav`, `.mp3`, `.flac`, `.opus`, `.m4a`, `.aac` и `.webm`. Доступность кодека при воспроизведении зависит от установленного в системе backend Qt Multimedia.

Звуки наследуются отдельно для каждого события. Дочерняя тема может переопределить только `navigate`, продолжая использовать остальные звуки родительской темы. Если в одной теме для события есть несколько поддерживаемых расширений, используется первое расширение из списка выше.
