...
- Папка тёмного варианта: `my_custom_theme`
- Папка светлого варианта: `my_custom_theme-light`

Если существуют обе папки, приложение показывает отдельный выбор варианта: `Dark`, `Light` и `Auto`. Если существует только одна папка, выбор варианта скрывается, а доступная тема используется напрямую.
...
Наследование применяется только к атрибутам стиля из `styles.py`. Шрифты, иконки, изображения и скриншоты не наследуются через `THEME_INHERITS`; для них остаётся встроенный откат к теме `standart`.
