...
Звуки наследуются отдельно для каждого события. Дочерняя тема может переопределить только `navigate`; отсутствующие события ищутся по цепочке родительских тем вплоть до `standart`. Если в одной теме для события есть несколько поддерживаемых расширений, используется первое расширение из списка выше.
