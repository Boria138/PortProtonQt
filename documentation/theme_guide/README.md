...
Sounds are inherited per event. A child theme can override only `navigate`; missing events are resolved through its parent chain up to `standart`. When more than one supported extension exists for the same event in one theme, the first extension in the list above is used.
